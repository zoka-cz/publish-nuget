...
class Action {
    ...
    _pushPackage(version, name) {
        ...
        this._executeInProcess(`dotnet pack ${this.includeSymbols ? "--include-symbols --include-source" : ""} --no-build -c Release ${this.projectFile} -o .`)

        const packages = fs.readdirSync(".").filter(fn => fn.endsWith("nupkg"))
        console.log(`Generated Package(s): ${packages.join(", ")}`)

        const pushCmd = `dotnet nuget push *.symbols.nupkg -s ${this.nugetSource}/v3/index.json -k ${this.nugetKey} --skip-duplicate`,
